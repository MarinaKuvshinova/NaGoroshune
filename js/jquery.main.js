...
*/	jcf.customForms.replaceAll();
	...
	$('.logo').bind('inview', function(event, isInView) {
		if (isInView) {
			setTimeout(function(){
	...
	$('.add-logo').bind('inview', function(event, isInView) {
		if (isInView) {
			setTimeout(function(){
				$('.add-logo').addClass('animated slideInDown');
			}, 0);
		}
	});
	$('.intro h1').bind('inview', function(event, isInView) {
		if (isInView) {
			setTimeout(function(){
				$('.intro h1').addClass('animated bounce');
			}, 1500);
		}
	});
	$('.intro .title').bind('inview', function(event, isInView) {
		if (isInView) {
			setTimeout(function(){
				$('.intro .title').addClass('animated bounce');
			}, 1500);
		}
	});
...
/*
	jQuery('.button-list li').contentPopup({
		mode: 'hover',
		openClass: 'hover-active',
		popup: '.popup-hover'
	});
*/
}
...
function initOpenClose() {
	jQuery('.info-container li').openClose({
...
*/
	...
	jQuery('.order-table tr').openClose({
		activeClass: 'active',
		opener: '.first-row',
		slider: '.second-row',
		animSpeed: 600,
...
/*
	jQuery('.add-menu').openClose({
		activeClass: 'active',
		opener: '>.opener',
		slider: '>.footer-holder',
		animSpeed: 400,
		effect: 'slide'
	});
*/
...
function initCarousel() {
	...
	jQuery('div.review').scrollGallery({
		mask: 'div.mask',
		slider: 'div.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
...
function initSameHeight() {
	jQuery('.promo-container').sameHeight({
		elements: '.name',
		flexible: true,
		multiLine: true
	});
...
function initCustomHover() {
	jQuery('.main-nav li').touchHover();
}


// cycle scroll gallery init
function initCycleCarousel() {
	jQuery('.pagination-gallery').scrollGallery({
		mask: '.mask',
		slides: '.slide',
		vertical: true, 
		step: 1,
	...
	jQuery('.gallery-col').scrollAbsoluteGallery({
		mask: '.gallery-slide',
		slider: '.slideset',
		slides: '.slide',
		pagerLinks: '.pagination-gallery a',
		maskAutoSize: true,
		autoRotation: false,
		switchTime: 7000,
		animSpeed: 500,
		onBeforeChange: function(obj){
			...
			var galleryAPI = jQuery('.pagination-gallery').data('ScrollGallery');
			galleryAPI.slides.each(function(i){
				var slide = jQuery(this);
				if(slide.find('a.active').length) galleryAPI.numSlide(i);
			});
		}
	});
	console.log(jQuery('.picture-gallery .pagination-gallery a').length, jQuery('.picture-gallery .gallery-slide .slide').length)
